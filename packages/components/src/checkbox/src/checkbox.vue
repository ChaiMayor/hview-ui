...
const model = computed({
  get() {
    return isGroup.value ? checkboxGroup && checkboxGroup.modelValue.value : props.modelValue;
  },
  set(val) {
    if (isGroup.value) {
...
const isDisabled = computed(() => {
  return isGroup.value ? (checkboxGroup && checkboxGroup.disabled.value) || props.disabled : props.disabled;
});
