<template>
  <div
    ...
    <template v-if="!props.controlsPosition">
      <span
        :class="['h-input-number__decrease', { 'is-disabled': disabledClass === 'decrease' }]"
        @click="decrease(step)">
      ...
      <span
        :class="['h-input-number__increase', { 'is-disabled': disabledClass === 'increase' }]"
        @click="increase(step)">
...
const blurInput = (e: FocusEvent) => {
  if (stepStrictly.value && step.value > 1) {
    ...
    location === "start" ? (numberValue.value = res[0]) : (numberValue.value = res[1]);
  }
  emits("blur", e);
  update();
};
...
const update = () => {
  nextTick(() => {
    if (isDisabled.value || readonly.value) return;
    numberValue.value <= min.value && ((disabledClass.value = "decrease"), (numberValue.value = min.value));
    numberValue.value >= max.value && ((disabledClass.value = "increase"), (numberValue.value = max.value));
