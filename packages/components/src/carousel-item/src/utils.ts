...
export const useCarouselItem = (props: CarouselItemProps, componentName: string) => {
  const carouselContext = inject(carouselContextKey)!;
  // instance
  const instance = getCurrentInstance()!;
  if (!carouselContext) {
    console.error(componentName, "usage: <h-carousel><h-carousel-item></h-carousel-item></h-carousel>");
  }

  if (!instance) {
    console.error(componentName, "compositional hook can only be invoked inside setups");
  }
