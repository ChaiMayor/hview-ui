...
export const useCarousel = (props: CarouselProps, emit: SetupContext<CarouselEmits>["emit"], componentName: string) => {
  ...
  } = useOrderedChildren<CarouselItemContext>(getCurrentInstance()!, "HCarouselItem");
  ...
  const hasLabel = computed(() => {
    return items.value.some((item: any) => item.props.label.toString().length > 0);
  });
  ...
  function setActiveItem(index: number | string) {
    if (isString(index)) {
      const filteredItems = items.value.filter((item: any) => item.props.name === index);
