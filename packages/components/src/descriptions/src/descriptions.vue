<template>
  <div class="h-descriptions" :class="descriptionKls">
    <div v-if="title || extra || $slots.title || $slots.extra" class="h-descriptions-header">
...
const getRows = () => {
  const children = flattedChildren(slots.default?.()).filter((node) => node?.type?.name === "HDescriptionsItem");
  const rows = [];
  let temp = [];
