<template>
  <div class="h-date-picker">
    ...
    <Teleport to="body">
      <div class="h-picker-body" v-if="visible === true && mode === 'date'" ref="calendarRef">
        <div class="h-picker-header">
          <span @click="selectYM(['year', -1])">&lt;&lt;</span>
          <span @click="selectYM(['month', -1])">&lt;</span>
          <span class="showInfo" @click="pickerYM('pickerYear')">{{ dateInfo[0] }}年</span>
          <span class="showInfo" @click="pickerYM('pickerMonth')">{{ dateInfo[1] }}月</span>
          <span @click="selectYM(['month', 1])">&gt;</span>
          <span @click="selectYM(['year', 1])">&gt;&gt;</span>
        </div>
...
const selectYM = (args: [string, number?, string?]) => {
  ...
  switch (args[0]) {
    ...
    case "monthMark":
      ...
      break;
    case "year":
      timestamp = dateTimePicker.cloneDate(time.value).setFullYear(year + args[1]);
      break;
    case "month":
      timestamp = dateTimePicker.cloneDate(time.value).setMonth(month - 1 + args[1]);
      break;
  }
