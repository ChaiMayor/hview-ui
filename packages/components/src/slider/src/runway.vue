...
const runwayClick = (e: MouseEvent) => {
  ...
  if (props.vertical) {
    site.value =
      offsetTop(slider_runway.value as HTMLDivElement) + (slider_runway.value as HTMLDivElement).offsetHeight - e.pageY;
  } else {
...
onMounted(() => {
  update();
  slider_runway.value && slider_runway.value!.addEventListener("click", runwayClick);
});
onUnmounted(() => {
  slider_runway.value && slider_runway.value!.removeEventListener("click", runwayClick);
});
defineExpose({
  update,
});
