          ...
          <li v-for="(file, index) in previewImgList">
            ...
            <div class="view">
              ...
              <span class="preview-delete-icon">
                <h-icon name="view" @click="openMask(index)" class="preview-icon"></h-icon>
                <h-icon name="delete" @click="removeImg(index)" class="deletecon"></h-icon>
              </span>
              <!-- 图片 -->
            ...
            <h-modal2 v-model:show="isModalVisible">
              ...
              <template v-slot:body> <img :src="modalImgUrl" alt="无法获取图片信息" class="modalImg" /></template>
            </h-modal2>
